...
export default {
  ...
  computed: {
    ...
    days () {
      ...
      let firstDayInMonthIndex = this.firstDayInMonthIndex(dObj);
      for(let f = 0; f < firstDayInMonthIndex; f++) {
        days.push({
          isDisabled: true,
          date: '',
          key: 'previous-'+f
  ...
  methods: {
    ...
    firstDayInMonthIndex(dObj) {
      let monthIndex = dObj.getMonth();
      let year = dObj.getFullYear();
      var day = new Date(`${year}-${monthIndex + 1}-01`).getDay();
      return day;
    },
    init () {
      if (this.value) {
